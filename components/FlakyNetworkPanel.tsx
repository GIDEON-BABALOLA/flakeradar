// // components/FlakyNetworkPanel.tsx
// //
// // Drop this into your existing tab setup.
// // It streams SSE from /api/flaky-network and renders commits live
...
// export default function FlakyNetworkPanel() {
...
//     const url = `/api/flaky-network?repoPage=${repoPage}&maxRepos=${maxRepos}`;
...
//         <span className="text-amber-400">network/concurrency</span> — useful for
...
//     a.download = "flaky-network-commits.csv";
//     a.click();
//   };
...
// type StarRange = typeof STAR_RANGES[number]["value"];
...
//   // Config
//   const [starRange, setStarRange] = useState<StarRange>("200000..*");
...
//     const url = `/api/flaky-network?starRange=${encodeURIComponent(starRange)}&maxRepos=${maxRepos}`;
//     const es = new EventSource(url);
//     esRef.current = es;
...
//             value={starRange}
//             disabled={loading}
//             onChange={(e) => setStarRange(e.target.value as StarRange)}
//             className="
//               bg-raised border border-line rounded-lg
//               font-mono text-[0.78rem] text-zinc-300
...
//             disabled={loading}
//             onChange={(e) =>
//               setMaxRepos(Math.min(100, Math.max(1, parseInt(e.target.value, 10) || 1)))
//             }
//             className="
//               w-24 bg-raised border border-line rounded-lg
//               font-mono text-sm text-zinc-300
...
//           <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse inline-block shrink-0" />
//           <span className="font-mono text-[0.68rem] text-zinc-500 truncate">
//             Scanning <span className="text-amber-400">{currentRepo}</span>
//             {currentLang && (
//               <span className="ml-1.5 px-1.5 py-0.5 rounded text-[0.58rem] bg-amber-500/10 text-amber-500 border border-amber-500/20">
//                 {currentLang}
//               </span>
...
//                 ))}
//               </div>
//               {(done || !loading) && <ExportButton commits={commits} />}
//             </>
//           )}
...
type SSEEvent =
  | { type: "commit"; data: CommitItem & { source: "commit" | "pr" } }
  | { type: "progress"; repo: string; language: string }
  | { type: "done"; total: number }
  | { type: "error"; message: string };
...
export default function FlakyNetworkPanel() {
  const [commits, setCommits]         = useState<(CommitItem & { source: "commit" | "pr" })[]>([]);
  const [currentRepo, setCurrentRepo] = useState<string | null>(null);
  const [currentLang, setCurrentLang] = useState<string>("");
  const [loading, setLoading]         = useState(false);
  const [done, setDone]               = useState(false);
        ...
        <div className="mt-7 pt-6 border-t border-line">
          ...
          {commits.length > 0 && (
            <>
              <div className="flex flex-col gap-2">
                {commits.map((commit, i) => (
                  <div key={`${commit.sha}-${i}`} className="relative">
                    <CommitCard item={commit} index={i} />
                    <span className={`
                      absolute top-3 right-8 font-mono text-[0.55rem] tracking-wider uppercase px-1.5 py-0.5 rounded
                      ${commit.source === "pr"
                        ? "bg-blue-500/10 text-blue-400 border border-blue-500/20"
                        : "bg-amber-500/10 text-amber-500 border border-amber-500/20"}
                    `}>
                      {commit.source === "pr" ? "PR body" : "commit msg"}
                    </span>
                  </div>
                ))}
              </div>
